...
- Direct per-recipient local socket for request/reply transport.
- SQLite transaction containing the certificate, pointers, and protocol state, never payload copies.
...
The prototype uses the recommended initial Adapter: Unix socket plus SQLite.

- **Unix socket + SQLite:** one recipient-finalized transaction gives clear acceptance, sequencing, request arbitration, and receipt reconciliation. It has the strongest locality and the least custom recovery code.
