...
**Delegation Policy**:
The effective rule selected by an ordinary Agent's immediate Spawner for that Agent's own future Agent-requested child activations: `disabled`, `approval-required`, or `autonomous`. An explicit SpawnSpec value overrides the Agent Definition default, and the protocol defaults to `approval-required`. The Workflow Owner has no Delegation Policy and delegates without policy evaluation or routine approval. A Moderator has no Delegation Policy and cannot create children. Approval coordinates duplicate-work prevention; it is not a security boundary or semantic duplicate-work detector.
_Avoid_: Spawning capability, Moderator authority, standing authorization
...
**Human Interrupt**:
The durable pause created when an ordinary Subagent calls `agent_ask_user({ question })` as the sole tool call in its assistant message. The required plain-text question is canonical in that tool call and is not parsed or semantically validated. The call atomically checkpoints the session, records one pending interrupt identified internally by the tool-call ID, enters `waiting(human)`, and settles. The human answers normally in that Subagent's pane; submission durably binds the input and clears `DECIDE`, but the interrupt remains a response-bound recovery obligation until resume commits the original tool call's result. An unclear response still resolves that interaction, so the Subagent asks again with a new call when needed. Workflow Owners and Moderators cannot create Human Interrupts. While awaiting response, other accepted actionable inputs remain ineligible and do not wake the Subagent. Human-wait duration alone never escalates.
_Avoid_: Agent Request, concurrently mutable dependency, model-decided resolution, generic wait marker

**Operation Dependency**:
An unresolved runtime operation that prevents an activation from safely continuing, settling, or completing. The runtime derives it from persisted work such as an in-flight tool call, ambiguous message acceptance, cancellation, ownership reconciliation, or tracked side-effect uncertainty; an Agent never creates it through a generic wait operation. Tool-call arguments remain opaque canonical call data, so a timeout passed to a tool controls only that tool and is not an oversight policy.
_Avoid_: Agent-declared wait, tool timeout, progress claim

**Operation Review Deadline**:
The runtime-owned limit on how long an Operation Dependency may remain unattended before operational judgment is required. Runtime policy supplies operation-specific timing and Workflow policy caps the unattended interval; Agent inputs, tool arguments, output, heartbeats, and process activity cannot extend or reset it. Expiry permits only deterministic side-effect-free reconciliation, then creates an Operational Incident if the operation remains unresolved or no safe deterministic choice exists. A Moderator may renew the deadline within Workflow policy with a durable rationale; renewal atomically closes that incident, while later expiry creates a fresh incident episode.
_Avoid_: Tool execution timeout, proof of failure, automatic cancellation, activity-based renewal

**Undeclared Settlement**:
A Subagent settlement with no terminal action, no declared Agent, Delegated Activation Approval, or operation dependency, and no Human Interrupt. It is a protocol error represented as `waiting(undeclared)`, never `waiting(human)`. The runtime delivers one durable Undeclared Settlement Notice per continuous episode. Completion or activation cancellation closes the episode; failure preserves it as recovery-pending work without granting another correction allowance. A declared dependency resets the allowance only after new input satisfies it. Another undeclared settlement creates an Operational Incident, except that a Moderator uses the existing Owner Handoff fallback rather than creating nested moderation.
...
Passive transport and session facts: `accepted` means the recipient durably owns the actionable message, while `delivered` means the message is committed to the recipient session for model consumption. Delivery timing (`steer | deferred`), transport status, and current inbox eligibility are independent: an accepted message may remain ineligible while active work or a Human Interrupt is preserved. A Request becomes `answered` when its correlated Answer is accepted and `resolved` when that Answer is delivered. There is no model-level read receipt.
_Avoid_: Agent lifecycle state, acknowledgement message, inbox eligibility

**Message Wake Rule**:
An actionable message schedules immediate work for an ordinarily waiting Agent and enters an active Agent according to its delivery timing. A pending Human Interrupt instead queues non-human input until the human-resume boundary. Signals may queue for an interrupted Agent and are rejected for an ended Agent. A Request from a direct Spawner or Workflow Owner may restart interrupted work; creating a new activation for an ended Agent additionally requires the explicit activating `agent_send` branch and its Activation Intent. An ordinary Agent's activating Request follows its Delegation Policy, while Workflow Owner delegation is policy-exempt. The same Request from a merely addressable peer is rejected.
_Avoid_: Addressability as resume authority, revival by Signal

**Independent Dependency Wakeup**:
Outside a Human Interrupt, delivery of any Answer starts its requester independently of other outstanding Requests. While a Human Interrupt is pending, accepted Agent and operation inputs remain ineligible without waking the Subagent; the human response resumes the suspended tool call and makes pending eligible inputs available at that resumed model boundary. Transport provides no `all` or `any` synchronization.
_Avoid_: Barrier, join policy

**Dependency Deadlock**:
A closed component of the Request dependency graph in which every participating Agent is durably waiting exclusively on unresolved Requests within that same component, with no active participant, eligible accepted actionable message, unresolved external or operation dependency, or recovery path capable of waking it. Request cycles are permitted and remain productive whenever any such source of progress exists.
...
One deduplicated episode where a runtime-confirmed operational condition blocks progress or safe cleanup, deterministic recovery or reconciliation is exhausted or cannot choose safely, and resolution requires temporary diagnostic visibility or lifecycle authority. Confirmed Dependency Deadlock, repeated Undeclared Settlement after its single correction, exhausted Automatic Recovery, persistent cancellation, ownership, acceptance, or side-effect uncertainty, and an unresolved Operation Dependency after its Operation Review Deadline and permitted reconciliation qualify. A Moderator's repeated Undeclared Settlement uses Owner Handoff fallback instead of creating another incident. Ordinary watch conditions, domain decisions, arbitrary elapsed time, Agent-selected tool timeouts, and failures with recovery still available do not.
...
**Incident Control**:
Temporary operational authority granted to a Moderator over non-Owner Agents within its Incident Scope. It may message them, interrupt active work, restart interrupted work, create recovery activations for failed Agents, cancel activations, and declare Recovery Abandonment. Every mutation is durably attributed to the incident with the Moderator's rationale. It cannot answer or cancel a Request for its owner, decide or cancel a Delegated Activation Approval, declare another Agent complete, alter identity, transcripts, Agent Definitions, Delegation Policy, tools, or capabilities, create any child, reparent Agents, transfer authority, widen Incident Scope, or control the Workflow Owner.
...
**Answer Finality**:
The first durably accepted Answer closes a Request's answer slot. Retrying the same Answer ID is idempotent, while a different later Answer is rejected; subsequent corrections require a new Signal or Request.
